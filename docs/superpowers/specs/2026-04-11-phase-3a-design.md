...
- **Dogfood**: `scripts/phase-3a-dogfood.sh` — 7-step exit criterion, запускается на {LV_DCP, TG_Proxy_enaibler_bot, TG_RUSCOFFEE_ADMIN_BOT}, результат — `docs/dogfood/phase-3a.md`.
...
Воспроизводимый bash script, для каждого project P ∈ {LV_DCP, TG_Proxy_enaibler_bot, TG_RUSCOFFEE_ADMIN_BOT}:
...
- 2026-04-11 — brainstorm session with Vladimir Lukin. Design points closed:
  ...
  - Dogfood projects {LV_DCP, TG_Proxy_enaibler_bot, TG_RUSCOFFEE_ADMIN_BOT} + 7-step exit: approved
